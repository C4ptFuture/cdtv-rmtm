...
AFLAGS=-m68000 -kick1hunks -Fhunkexe -nosym -ldots
#AFLAGS=-no-opt -Fhunkexe -nosym -ldots
DEFINES=-DVERSION=$(VERSION) -DREVISION=$(REVISION) -D$(ENVIRONMENT)



.PHONY: check-env clean builddir bumprev


$(PROJECT): deps 
	$(VASM) $(DEFINES) $(AFLAGS) -I$(NDK_INC_ASM) -I$(CDTV_INC_ASM) $(SRC)/$(PROJECT).asm -o $(BUILD)/$(PROJECT)-$(VERSION).$(REVISION)-$(ENVIRONMENT).ld
#	$(VLINK) -s -bamigahunk -L$(NDK_LIB) -lamiga -Bstatic \
		$(BUILD)/$(PROJECT)-$(VERSION).$(REVISION)-$(ENVIRONMENT).o -o $(BUILD)/$(PROJECT)-$(VERSION).$(REVISION)-$(ENVIRONMENT).ld
ifeq ($(PUSH),yes)
		@echo "---> Uploading artifact to OCI registry and signing.."
		cosign upload blob -f $(BUILD)/$(PROJECT)-$(VERSION).$(REVISION)-$(ENVIRONMENT).ld harbor.spacetequila.com/cdtv/binaries/$(PROJECT):$(VERSION).$(REVISION)-$(ENVIRONMENT)
		cosign sign --key /cosign.key -y harbor.spacetequila.com/cdtv/binaries/$(PROJECT):$(VERSION).$(REVISION)-$(ENVIRONMENT)
		@echo "Done."
endif


deps: check-env bumprev createrev builddir
...
bumprev:
#	tools/bumprev/bumprev.sh VERSION
...
diffcheck: check-env
	@diff "${ORIGINAL_ARTIFACT}" "$(BUILD)/$(PROJECT)-$(VERSION).$(REVISION)-$(ENVIRONMENT).ld" && printf "*****************  DIFF OK **********************\n"|| printf "\n\n*****************  DIFF FAILED **********************\n\n\n"
